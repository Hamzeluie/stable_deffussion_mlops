...
def renew_vae_attention_paths(old_list, n_shave_prefix_segments=0):
    ...
    for old_item in old_list:
        ...
        if new_item.find("to_") != -1:
            new_item = new_item.replace("q.weight", "q.weight")
            new_item = new_item.replace("q.bias", "q.bias")

            new_item = new_item.replace("k.weight", "k.weight")
            new_item = new_item.replace("k.bias", "k.bias")

            new_item = new_item.replace("v.weight", "v.weight")
            new_item = new_item.replace("v.bias", "v.bias")
        else:
...
def download_from_original_stable_diffusion_ckpt(
    ...
    if model_type is None:
        ...
        print(f"no `model_type` given, `model_type` inferred as: {model_type}")
